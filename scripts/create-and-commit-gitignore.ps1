...
# Ensure running in repo root (simple heuristic: repo folder name contains 'ict-trading-bot')
...
# Branch name for automated changes
...
# Push changes (in Actions runner with persist-credentials this should work)
try {
    git push --set-upstream origin $branch 2>$null
    Write-Output "Pushed branch $branch to origin."
} catch {
    Write-Warning "Push failed (this may be expected in some runners). create-pull-request action will create the PR."
}
